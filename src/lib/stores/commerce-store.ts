...
interface CommerceActions {
  ...
  // Inventory actions
  createMovement: (movement: Omit<InventoryMovement, "id" | "createdAt">) => void;
  adjustStock: (
    productId: string,
...
const mockProducts: Product[] = [
  ...
  },
  {
    ...
    images: [
      {
        id: "img-3",
        url: "https://picsum.photos/seed/coffee/400/400",
        alt: "Coffee Beans",
        isPrimary: true,
      },
    ],
  ...
  },
  {
    ...
    images: [
      {
        id: "img-5",
        url: "https://picsum.photos/seed/wallet/400/400",
        alt: "Wallet",
        isPrimary: true,
      },
    ],
  ...
  },
  {
    ...
    images: [
      {
        id: "img-6",
        url: "https://picsum.photos/seed/cable/400/400",
        alt: "USB-C Cable",
        isPrimary: true,
      },
    ],
  ...
  },
  {
    ...
    images: [
      {
        id: "img-7",
        url: "https://picsum.photos/seed/jeans/400/400",
        alt: "Jeans",
        isPrimary: true,
      },
    ],
  ...
  },
  {
    ...
    images: [
      {
        id: "img-8",
        url: "https://picsum.photos/seed/tea/400/400",
        alt: "Green Tea",
        isPrimary: true,
      },
    ],
    ...
    (set, get) => ({
      ...
      actions: {
        ...
        createOrder: (order) => {
          ...
          const todayOrders = state.orders.filter((o) => o.orderNumber.includes(dateStr)).length;
        ...
        processRefund: (orderId, itemIds) => {
          ...
          const refundCost = itemsToRefund.reduce((sum, i) => sum + i.unitCost * i.quantity, 0);
...
export const useActiveProducts = () =>
  useCommerceStore(useShallow((state) => state.products.filter((p) => p.status === "active")));
...
export const useOrdersByStatus = (status: OrderStatus) =>
  useCommerceStore(useShallow((state) => state.orders.filter((o) => o.status === status)));

export const useOrdersBySource = (source: OrderSource) =>
  useCommerceStore(useShallow((state) => state.orders.filter((o) => o.source === source)));
...
export const useTotalOrderCount = () =>
  useCommerceStore((state) => state.orders.filter((o) => o.status !== "cancelled").length);
...
export const useCustomerOrders = (customerId: string) =>
  useCommerceStore(useShallow((state) => state.orders.filter((o) => o.customerId === customerId)));
