 ...
 *
 * Shared by POST /api/auth/mark-verified, which the verify-email page calls
 * after a code, and /auth/callback, which covers OAuth and email links. Both
 * end in a proved address and both must write the same flag, or whichever one
 * was forgotten leaves that route's users stuck behind the middleware gate.
 *
