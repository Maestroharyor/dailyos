...
describe("computeOrderTotals with an agreed tax", () => {
  // An offline sale is priced against the settings as they stood, printed, and
  // paid for. If the merchant changes the tax rate before it syncs, repricing
  // it makes the shop's record disagree with the receipt in the customer's
  // hand over money that has already changed hands.
  it("uses the agreed figure instead of the live rate", () => {
    const totals = computeOrderTotals({
      subtotal: 1000,
      taxRate: 15,
      agreedTax: 75,
    });
    expect(totals.tax).toBe(75);
  });

  it("carries the agreed tax through to the total", () => {
    const totals = computeOrderTotals({
      subtotal: 1000,
      discount: 100,
      taxRate: 15,
    ...
    // 1000 - 100 + 67.5
    expect(totals.total).toBe(967.5);
  });

  it("prices from the live rate when nothing was agreed", () => {
    const totals = computeOrderTotals({ subtotal: 1000, taxRate: 7.5 });
    expect(totals.tax).toBe(75);
  });

  // Zero is a real agreed figure — a tax-exempt sale — and must not fall
  // through to the live rate the way a missing value does.
  it("treats an agreed zero as agreed, not as absent", () => {
    const totals = computeOrderTotals({
      subtotal: 1000,
      taxRate: 15,
      agreedTax: 0,
  ...
  it("refuses a negative agreed figure", () => {
    const totals = computeOrderTotals({
      subtotal: 1000,
      taxRate: 15,
      agreedTax: -50,
  ...
  // The discount cap and the shipping rule are the same arithmetic either way:
  // only where the tax figure comes from changes.
  it("still caps the discount at the subtotal and adds shipping", () => {
    const totals = computeOrderTotals({
      subtotal: 500,
      discount: 900,
      taxRate: 15,
